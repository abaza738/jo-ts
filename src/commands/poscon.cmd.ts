...
export abstract class POSCONCommand {
  ...
  async poscon(interaction: CommandInteraction) {
    ...
    .addFields(
      { name: "Pilots", value: `${onlineData?.totalPilots ?? 0}`, inline: true },
      { name: "ATC", value: `${onlineData?.totalAtc ?? 0}`, inline: true },
      { name: "Upcoming ATC", value: onlineData?.upcomingAtc.slice(0,5).map(uATC => `${ moment(uATC.start).utc().format("Do | HHmm") }z - ${toZuluTime(uATC.end)}: **${ uATC.telephony ?? uATC.position }**`).join('\n') ?? 'No Upcoming ATC!' },
      { name: "Last updated", value: `<t:${moment(onlineData?.lastUpdated).unix()}:R>` }
    );
  ...
  async flight(
    ...
    .addFields(
      ...
      { name: "ALT", value: `${Math.round(selectedFlight.position.pressure_alt)} ft. MSL`, inline: true },
      { name: "HDG", value: `${Math.round(selectedFlight.position.true_hdg)}°`, inline: true },
      { name: "GS", value: `${Math.round(selectedFlight.position.gs_kt)}kts.`, inline: true }
    );

    if (selectedFlight.freq) {
      embed
      .addFields(
        { name: "VHF1", value: `${KHzToMHz(+selectedFlight.freq.vhf1) ?? '--'} MHz`, inline: true },
        { name: "VHF2", value: `${KHzToMHz(+selectedFlight.freq.vhf2) ?? '--'} MHz`, inline: true }
      );
    }

    embed.addFields({ name: "Status", value: await flightStatus(selectedFlight) });

    if (selectedFlight.flightplan) {
      embed
      .addFields(
        { name: "Time", value: `Scheduled Time of Departure ${toZuluTime(selectedFlight.flightplan.std!)}
        ...
        { name: "Route", value: `**${selectedFlight.flightplan.dep}** → **${selectedFlight.flightplan.dest}**\`\`\`\n${selectedFlight.flightplan.route}\`\`\`` },
        { name: "PAX", value: `${selectedFlight.flightplan.persons} passengers onboard.` }
      );
    } else {
      embed.addFields({ name: "Flight Plan", value: "No active flight plan correlated to this target." });
    }
    embed.addFields({ name: "PIC", value: `[${selectedFlight.userName}](${constants.POSCON.URLS.PROFILE}${selectedFlight.userId})`});
